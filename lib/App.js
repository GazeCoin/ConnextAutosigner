...
var mode = process.env.MODE || 'adbot'; // or 'proxybot'
...
var App = function () {
  ...
  _createClass(App, [{
    ...
    value: async function init() {
      ...
      var storeFile = "./config/" + process.argv[2] + "/.store";
    ...
    value: async function startWsServer() {
      ...
      router.get('/balance', function (req, res) {
        ...
        var tokenBal = web3.utils.fromWei(state.channelState ? state.channelState.balanceTokenUser : "0");
        res.json({ account: state.address, balance: tokenBal });
      });

      router.route('/pay/:payto/:amount/:locid?').post(async function (req, res) {
        ...
        var result = await _this.sendPayment(req.params.payto, req.params.amount);
