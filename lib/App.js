...
var App = function () {
  ...
  _createClass(App, [{
    ...
    value: async function setConnext(rpc, mnemonic) {
      ...
      try {
        var connext = await _connext2.createClient(opts);
    ...
    value: async function setDepositLimits() {
      ...
      var minDeposit = _connext2.default.Currency.WEI(totalDepositGasWei, function () {
        return getExchangeRates(connextState);
      });

      var maxDeposit = _connext2.default.Currency.DEI(CHANNEL_DEPOSIT_MAX, function () {
        return getExchangeRates(connextState);
      });
